...
// ── Utilities ────────────────────────────────────────────────────
// Credibility: green / amber / red
function scoreColor(s) { return s>=75?"#4ade80":s>=50?"#c8a96e":"#f87171"; }
function avatarBg(s) { return s>=75?"#1a3a2a":s>=50?"#3a2a10":"#3a1010"; }

// Fact/Opinion: blue → amber (no red/green)
function ioColor(s) {
  const b=[74,144,210], a=[200,169,110];
  return `rgb(${Math.round(b[0]+(a[0]-b[0])*s)},${Math.round(b[1]+(a[1]-b[1])*s)},${Math.round(b[2]+(a[2]-b[2])*s)})`;
}
...
// ── Mini spider (no labels) — for poster cred swap ────────────────────────
function MiniSpider({ dims, size=100 }) {
  const keys=Object.keys(dims), n=keys.length;
  const cx=size/2, cy=size/2, mR=size*0.33;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {[1,2,3,4].map(l=>{ const r=(l/4)*mR; const pp=keys.map((_,i)=>{ const a=angle(i); return `${cx+r*Math.cos(a)},${cy+r*Math.sin(a)}`; }).join(" "); return <polygon key={l} points={pp} fill="none" stroke="#1e1e1e" strokeWidth="0.5"/>; })}
      {keys.map((_,i)=>{ const p=pt(i,1); return <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="#1a1a1a" strokeWidth="0.5"/>; })}
      <polygon points={polyPts} fill={`${col}33`} stroke={col} strokeWidth="2"/>
      {keys.map((k,i)=>{ const p=pt(i,dims[k]/100); return <circle key={k} cx={p.x} cy={p.y} r={3} fill={scoreColor(dims[k])} stroke="#0e0e0e" strokeWidth={1.5}/>; })}
    </svg>
  );
}

// ── Full spider (with labels) — for reposter cred swap ──────────────────────
function FullSpider({ dims, size=176 }) {
  const keys=Object.keys(dims), n=keys.length;
  const cx=size/2, cy=size/2, mR=size*0.296, lR=mR+41;
  const avg=Object.values(dims).reduce((s,v)=>s+v,0)/n;
  const col=scoreColor(avg);
  const angle=(i)=>(Math.PI*2*i/n)-Math.PI/2;
  const pt=(i,v)=>({ x:cx+v*mR*Math.cos(angle(i)), y:cy+v*mR*Math.sin(angle(i)) });
  const polyPts=keys.map((k,i)=>{ const p=pt(i,dims[k]/100); return `${p.x},${p.y}`; }).join(" ");
  return (
    <div style={{display:"flex",justifyContent:"center",padding:"44px 24px 32px"}}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{overflow:"visible"}}>
        {[1,2,3,4].map(l=>{ const r=(l/4)*mR; const pp=keys.map((_,i)=>{ const a=angle(i); return `${cx+r*Math.cos(a)},${cy+r*Math.sin(a)}`; }).join(" "); return <g key={l}><polygon points={pp} fill="none" stroke="#1e1e1e" strokeWidth="0.5"/><text x={cx+4} y={cy-r+5} fill="#2a2a2a" fontSize={11} fontFamily="Palatino,serif">{l*25}</text></g>; })}
        {keys.map((_,i)=>{ const p=pt(i,1); return <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="#1a1a1a" strokeWidth="0.5"/>; })}
        ...
        {keys.map((k,i)=>{ const lx=cx+lR*Math.cos(angle(i)), ly=cy+lR*Math.sin(angle(i)); return <g key={k}><text x={lx} y={ly-7} textAnchor="middle" fill="#888" fontSize={13} fontFamily="Palatino,serif" fontStyle="italic">{DIM_LABELS[k]}</text><text x={lx} y={ly+16} textAnchor="middle" fill={scoreColor(dims[k])} fontSize={22} fontFamily="Palatino,serif">{dims[k]}</text></g>; })}
      </svg>
    </div>
  );
}
...
export default function PostFeed() {
  const [posterSpider,setPosterSpider]=useState(false);
  const [reposterSpider,setReposterSpider]=useState(false);
    ...
    <div style={{minHeight:"100vh",background:"#0a0a0a",fontFamily:"Palatino,'Book Antiqua',serif",padding:"32px 20px",display:"flex",justifyContent:"center"}}>
      <div style={{width:"100%",maxWidth:640}}>
        <div style={{marginBottom:18}}>
          <div style={{fontSize:14,letterSpacing:".25em",color:"#444",textTransform:"uppercase",marginBottom:5}}>Radical Moderate · feed</div>
        ...
        <div style={{background:"#0e0e0e",border:"0.5px solid #252525",borderRadius:8,overflow:"hidden"}}>
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:14,padding:"20px 22px 16px"}}>
            ...
            <div style={{flex:1,minWidth:0}}>
              <div style={{fontSize:22,color:"#ddd",marginBottom:5}}>{POSTER.name} <span style={{fontSize:18,color:"#555",fontStyle:"italic"}}>{POSTER.handle}</span></div>
            ...
            {/* Cred pill — swaps with mini spider */}
            <div onClick={()=>setPosterSpider(v=>!v)} style={{flexShrink:0,background:"#1a1010",border:`1px solid ${scoreColor(POSTER.overall)}33`,borderRadius:6,padding:"13px 14px",width:128,textAlign:"center",boxSizing:"border-box",cursor:"pointer"}}>
              {!posterSpider ? (
                <>
                  <div style={{fontSize:44,color:scoreColor(POSTER.overall),lineHeight:1,fontFamily:"Palatino,serif"}}>{POSTER.overall}</div>
                  <div style={{fontSize:13,color:"#663333",fontStyle:"italic",marginTop:4}}>credibility</div>
                  <div style={{fontSize:11,color:"#2e1515",fontStyle:"italic",marginTop:9}}>tap to expand</div>
                </>
              ) : (
                <>
                  <div style={{fontSize:10,color:"#3a3a3a",letterSpacing:".12em",textTransform:"uppercase",marginBottom:6}}>breakdown</div>
                  <div style={{display:"flex",justifyContent:"center"}}><MiniSpider dims={POSTER.dims} size={100}/></div>
                  <div style={{fontSize:11,color:"#2e1515",fontStyle:"italic",marginTop:6}}>tap to restore</div>
                </>
              )}
            </div>
          </div>

          {/* Post text — blue→amber Fact/Opinion */}
          ...
          <div style={{padding:"0 22px 16px",display:"flex",gap:18,alignItems:"center",flexWrap:"wrap"}}>
            {[["Fact",0],["Mixed",.5],["Opinion",1]].map(([l,s])=>(
              <div key={l} style={{display:"flex",alignItems:"center",gap:7}}>
                <div style={{width:9,height:9,borderRadius:"50%",background:ioColor(s)}}/>
                <span style={{fontSize:16,color:"#555",fontStyle:"italic"}}>{l}</span>
              </div>
            ))}
            <span style={{fontSize:15,color:"#3a3a3a",fontStyle:"italic",marginLeft:"auto"}}>Fact / Opinion</span>
          </div>
          ...
          <div style={{padding:"18px 22px"}}>
            ...
            {/* Avg reposter cred — swaps with full spider */}
            <div onClick={()=>setReposterSpider(v=>!v)} style={{background:"#111",border:"0.5px solid #252525",borderRadius:6,padding:"18px 22px",marginBottom:14,cursor:"pointer"}}>
              {!reposterSpider ? (
                <>
                  <div style={{fontSize:16,color:"#888",fontStyle:"italic",marginBottom:8}}>Average reposter credibility score</div>
                  <div style={{fontSize:52,color:scoreColor(avgCred),lineHeight:1}}>{avgCred}</div>
                  <div style={{fontSize:15,color:"#555",fontStyle:"italic",marginTop:8}}>{reposters.length} reposters · tap to expand</div>
                </>
              ) : (
                <>
                  <div style={{fontSize:13,color:"#3a3a3a",letterSpacing:".14em",textTransform:"uppercase",marginBottom:4}}>Avg reposter · credibility breakdown</div>
                  <FullSpider dims={REPOSTER_DIMS}/>
                  <div style={{fontSize:14,color:"#555",fontStyle:"italic",textAlign:"center"}}>tap to restore</div>
                </>
              )}
            </div>
            ...
            {showRaters&&<div style={{marginTop:10,display:"flex",flexDirection:"column",gap:6}}>{RATERS.map((r,i)=><RaterCard key={i} r={r}/>)}</div>}
          </div>
          ...
          <div style={{padding:"18px 22px 22px"}}>
            <div style={{fontSize:14,letterSpacing:".2em",color:"#3a3a3a",textTransform:"uppercase",marginBottom:4}}>Perspective check</div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:14}}>
              <div style={{background:"#0c0c0c",border:"0.5px solid #1e1e1e",borderLeft:"3px solid #4a7ab5",borderRadius:0,padding:17}}>
                <div style={{fontSize:13,letterSpacing:".12em",color:"#4a7ab5",textTransform:"uppercase",marginBottom:10}}>Someone unlike you — nearby</div>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                  ...
                  <div><div style={{fontSize:19,color:"#ccc"}}>Sarah K.</div><div style={{fontSize:15,color:"#555",fontStyle:"italic"}}>Conservative · Chicago, IL · cred <span style={{color:"#4ade80"}}>83</span></div></div>
                </div>
                <div style={{fontSize:13,color:"#4a7ab5",fontStyle:"italic",marginBottom:10}}>0.4 mi from the original poster</div>
              ...
              <div style={{background:"#0c0c0c",border:"0.5px solid #1e1e1e",borderLeft:"3px solid #c8a96e",borderRadius:0,padding:17}}>
                <div style={{fontSize:13,letterSpacing:".12em",color:"#c8a96e",textTransform:"uppercase",marginBottom:10}}>Someone like you</div>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                  ...
                  <div><div style={{fontSize:19,color:"#ccc"}}>James T.</div><div style={{fontSize:15,color:"#555",fontStyle:"italic"}}>Progressive · Portland, OR · cred <span style={{color:"#c8a96e"}}>69</span></div></div>
                </div>
                <div style={{fontSize:13,color:"#555",fontStyle:"italic",marginBottom:10}}>Shares your background</div>
        ...
        <div style={{marginTop:14,fontSize:15,color:"#444",fontStyle:"italic",textAlign:"center"}}>Cooling period active · reactions visible in 14h · credibility scores update continuously</div>
      </div>
    </div>
  );
}
